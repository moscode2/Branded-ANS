...
interface Article { id: string; slug: string; title: string; summary: string; category: string; date: string; read_time?: string; body?: string; author?: string; cover_image?: string; }
    ...
    <>
      ...
      <section className="pt-36 pb-12 border-b border-cyan/5">
        <div className="max-w-3xl mx-auto px-6">
          <Link href="/insights"
            className="inline-flex items-center gap-2 text-[0.68rem] tracking-[0.12em] uppercase text-muted hover:text-cyan transition-colors mb-10">
            ← Back to Insights
          </Link>

          {/* Cover image */}
          {article.cover_image && (
            <div className="mb-8 rounded overflow-hidden border border-cyan/10">
          ...
          <div className="flex items-center gap-3 mb-6">
            ...
            {article.read_time && (
              <>
                ...
                <span className="text-[0.68rem] text-muted">{article.read_time}</span>
              </>
            )}
          </div>
          ...
          <p className="text-lg text-muted leading-relaxed font-display italic">
            {article.summary}
          </p>
          ...
          {article.author && (
            <div className="mt-6 flex items-center gap-3">
              <div className="w-7 h-7 rounded-full bg-cyan/10 border border-cyan/25 flex items-center justify-center">
                <span className="text-[9px] text-cyan font-mono font-bold">
      ...
      <div className="max-w-3xl mx-auto px-6 py-8">
        <div className="flex items-center gap-4">
          ...
          <span className="text-[0.62rem] tracking-[0.2em] uppercase text-muted/50">
            Africa Narrative Signals · Research Analysis
          </span>
        </div>
      </div>
      ...
      <section className="pb-20">
        <div className="max-w-3xl mx-auto px-6">
          {article.body
            ...
            : (
              <div className="prose-ans">
                <p>Full article content appears here. Use the admin dashboard to add content to this insight.</p>
              </div>
            )
          }
          ...
          <div className="mt-14 pt-8 border-t border-cyan/8 flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-sand font-display font-600 tracking-wide">
                {article.author ?? "Africa Narrative Signals"}
              </p>
            </div>
            <Link href="/insights"
              className="text-[0.68rem] tracking-[0.12em] uppercase text-muted hover:text-cyan transition-colors">
              ← All Insights
            </Link>
          </div>
        </div>
